...
Except on the easier Fashion MNIST, the dynamic loss reduced performance
variance and also generally resulted in improved performance relative to
the standard cross-entropy loss. However, it should be noted that these
performances come from short training on the bootstrapped base training sets.
It remains to be seen whether the dynamic loss would improve performance
when training data is increased and training is more typical (100-200 epochs
instead of 50).
...
|-------------:|---------------|----------|----------|--------|--------|
|     **Data** | **Threshold** |          |          |        |        |
|      CIFAR10 | None          | 0.847316 | 0.014557 | 0.8102 | 0.8747 |
...
|     CIFAR100 | None          | 0.574636 | 0.021818 | 0.4973 | 0.6108 |
...
| FashionMNIST | None          | 0.930534 | 0.001941 | 0.9261 | 0.9350 |
...
constant does not affect the derivative.  Ignoring floating point issues, this
means it doesn't matter if we make our scaling factor 0.1, 69, or 12093.92: the
...
#### Cross-Entopy Loss Behaviour on Correct / Incorrect Predictions
...
#### Dynamic Loss Behaviour on Correct / Incorrect Predictions
...
gradients in case (1). Thus we need only consider what happens in case (2).

When the **prediction is incorrect and _over_-confident**, i.e. there is a
softmax value $> \tau$ on the wrong class, then **gradients are identical to
those from the cross-entropy**, since softmax values on the incorrect class do
not contribute to gradients in the cross-entropy loss, and since the dynamic
loss also sets these gradients to zero.

Where the **prediction is both confident ($> \tau$) and correct, the
gradient of the cross-entropy is non-zero, but the gradient of the dynamic loss
is zero**, by definition. This is the only case where the loss functions differ.
We can see this in simulations below.

#### Simulations of Dynamic Loss Gradient Behaviour

A simulation ([repo
script](https://github.com/stfxecutables/dynamic_loss/blob/master/scripts/loss_demonstration.py))
shows that the dynamic loss differs from the cross-entropy loss only in that it
*prevents* the model from updating from correct predictions above the threshold
$\tau$. The simulation looks at the weights of a linear layer (we can pretend
it is the final linear classification layer) and how they change when given an
input, and the loss is computed so that gradients back-prop.

In this simulation, we also define an additional "soft dynamic loss",
which is defined (in various equivalent forms, as per above) as:
...
which would zero gradients on these values). We will see this soft dynamic loss
behaves more similarly to the previous manuscript description.
...
behaviour from the cross-entropy loss**. In this case, the soft dynamic loss
has gradients 0.1 times the size of the cross-entropy gradients.
...
Label smoothing is a regularization method, and thus the similarity of the
dynamic loss behaviour may also explain the superiour observed performance on
the smaller base training data - the dynamic loss may be preventing overfitting.
